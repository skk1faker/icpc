...
{
  int val,num;
  forbid_val(int val, int num):val(val),num(num){}
  bool operator < (const forbid_val &temp)const {
    return this->num < temp.num;
  }
};

void dfs(int finish,set<forbid_val> forbid) {
  if (finish == n) {
    for(int i = 0;i < n;i++){
      if(!check(i)){
        return;
  ...
  for(set<forbid_val>::iterator iter = forbid.begin();iter != forbid.end();iter++){
    if(iter->num == finish){
      forbid.erase(iter);
    }
  }
  for (int st = 0; st < 2 * m + 2; st++) {
    int stat[4][2];
    digit2status(stat, st);
    int temp_vis[4];
    for(int i = 0;i < m;i++){
      temp_vis[i] = vis[matrix_[finish][stat[i][0]][stat[i][1]]];
      if(forbid.find(forbid_val(matrix_[finish][stat[i][0]][stat[i][1]], 0)) != forbid.end())continue;
      vis[matrix_[finish][stat[i][0]][stat[i][1]]] = 1;
    }
    int forbid2posi = -1;
    bool pre_check;
    bool now_check = true;
    ...
    if(forbid2posi == -1){
      ...
      for(int i = 0;i < m;i++){
        if(temp_vis[i] == 1 && forbid.find(forbid_val(matrix_[finish][stat[i][0]][stat[i][1]], 0)) != forbid.end())continue;
        vis[matrix_[finish][stat[i][0]][stat[i][1]]] = temp_vis[i];  
        set<forbid_val> forbid_copy = forbid;
        forbid_copy.insert(forbid_val(matrix_[finish][stat[i][0]][stat[i][1]], forbid2posi));
        dfs(finish + 1, forbid_copy);
      }
    }
    for(int i = 0;i < m;i++){
      vis[matrix_[finish][stat[i][0]][stat[i][1]]] = temp_vis[i];  
